...
class RecipeBox extends React.Component {
  ...
  const recipes = this.props.recipes.map( (recipe, i) => {
    return <li  key={i} id={i}><span className="title">{recipe.title}</span><input className="del" type="button" onClick={this.deleteRecipe} value="&#xf014;"/><input className="edit" type="button" value="Edit" onClick={this.props.handleEdit} /><ul className="ingredients">{recipe.ingredients.map(function(item,i){return <li key={i}>{item}</li>})}</ul></li>
		});
    return (
      <ul className="recipe">
        {recipes}
